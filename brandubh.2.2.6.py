...
class BrandubhGame:
    def __init__(self):
        ...
        self.volume = 0.5          # Default volume setting (50%)
    ...
    def play_sound(self, key, stop_first=False):
        ...
        if sound:
            if stop_first:
                sound.stop()
            # Dynamically push the user's volume preference right before playing
    ...
    def set_volume(self, selected_item, value):
        """Update runtime audio mix scaling profile."""
        self.volume = value
        # Play a quick test sound profile feedback if changing volume on a live mixer interaction
        self.play_sound("move", stop_first=True)
    ...
    def create_menu(self):
        ...
        menu.add.selector(
            "Volume: ",
            [("Mute", 0.0), ("Low", 0.2), ("Med", 0.5), ("High", 0.8), ("Max", 1.0)],
            default=2,  # Matches index of ("Med", 0.5)
    ...
    def get_valid_moves(self, row, col):
        ...
        for dr, dc in directions:
            ...
            while 0 <= r < BOARD_SIZE and 0 <= c < BOARD_SIZE:
                ...
                # Prevent suicide moves safely: only check sandwiching if adjacent squares are on the board
                sandwiched_h = False
                if 0 <= c - 1 and c + 1 < BOARD_SIZE:
                    if self.board[r][c - 1] in opponents and self.board[r][c + 1] in opponents:
                        sandwiched_h = True

                sandwiched_v = False
                if 0 <= r - 1 and r + 1 < BOARD_SIZE:
                    if self.board[r - 1][c] in opponents and self.board[r + 1][c] in opponents:
                        sandwiched_v = True
